...
public:
  ...
  struct ethernet_hdr* validateEther(const Buffer& packet, const std::string& inIface) const;
  struct arp_hdr* validateArp(const Buffer& packet) const;
  void processArp(const Buffer& packet,const std::string& inIface);
  void sendArpReply(const Buffer& packet,const std::string& inIface);
  void processArpReply(const Buffer& packet);
  struct ip_hdr* validateIPv4(const Buffer& packet) const;
  void processIPv4(const Buffer& packet,const std::string& inIface);
  void processICMP(const Buffer& packet,const std::string& inIface);
  void sendPortUnreachable(const Buffer& packet,const std::string& inIface);
  ...
  struct icmp_hdr* validateICMP(const Buffer& packet);
  void sendHostUnreachable(const Buffer&packet);
